  ...
  return (
    <ThemeGlobalSimple.Provider value={{ searchModal }}>
      <div
        ...
        {/* 外层容器：左右内边距从 md:px-24 改为 md:px-8，显著缩窄 */}
        <div className='flex flex-1 mx-auto overflow-hidden py-8 md:p-0 md:max-w-7xl md:px-8 w-screen'>
          <div className='overflow-hidden md:mt-8 flex-1'>
            {/* 内层容器：左右内边距从 md:px-2 改为 md:px-1，让内容更靠边 */}
            <div
              id='container-inner'
              className='h-full w-full md:px-1 overflow-y-auto scroll-hidden relative'>
...
// 以下所有函数（LayoutIndex, LayoutPostList, LayoutSearch, groupArticlesByYearArray, LayoutArchive, LayoutSlug, Layout404, LayoutCategoryIndex, LayoutTagIndex, 导出）与您原代码完全一致，这里省略避免重复。
// 请从您当前项目中复制这些函数保持不变，或者确保文件末尾有完整的导出。
// 为方便您，我已将完整的后续函数附在下面，您只需替换 LayoutBase 部分。
