import { AdSlot } from '@/components/GoogleAdsense'
import { siteConfig } from '@/lib/config'
import { useGlobal } from '@/lib/global'
import SmartLink from '@/components/SmartLink'
import { useRouter } from 'next/router'
import { useMemo } from 'react'
import CONFIG from '../config'
import { BlogItem } from './BlogItem'

export default function BlogListPage(props) {
  const { page = 1, posts, postCount } = props
  const router = useRouter()
  const { NOTION_CONFIG, searchKeyword } = useGlobal()
  const POSTS_PER_PAGE = siteConfig('POSTS_PER_PAGE', null, NOTION_CONFIG)
  const totalPage = Math.ceil(postCount / POSTS_PER_PAGE)
  const currentPage = +page

  // 搜索过滤
  const filteredPosts = useMemo(() => {
    if (!searchKeyword) return posts
    const keyword = searchKeyword.toLowerCase()
    return posts.filter(post => {
      const titleMatch = post.title?.toLowerCase().includes(keyword)
      const summaryMatch = post.summary?.toLowerCase().includes(keyword)
      return titleMatch || summaryMatch
    })
  }, [posts, searchKeyword])

  const showPrev = currentPage > 1 && !searchKeyword
  const showNext = page < totalPage && !searchKeyword
  const pagePrefix = router.asPath
    .split('?')[0]
    .replace(/\/page\/[1-9]\d*/, '')
    .replace(/\/$/, '')
    .replace('.html', '')

  // 有搜索词但没有结果时的提示
  if (searchKeyword && filteredPosts.length === 0) {
    return (
      <div className='w-full px-1 md:px-1 mt-4'>
        <div className="text-center py-12 text-gray-500 dark:text-gray-400">
          <i className="fas fa-search text-4xl mb-2"></i>
          <p>没有找到与“{searchKeyword}”相关的文章或页面</p>
        </div>
      </div>
    )
  }

  return (
    <div className='w-full px-1 md:px-1 mt-4'>
      <div id='posts-wrapper' className='grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-10 items-start'>
        {filteredPosts.map((p, index) => (
          <BlogItem key={p.id} post={p} />
        ))}
      </div>

      {!searchKeyword && (
        <div className='flex justify-between text-xs mt-12'>
          <SmartLink
            href={{
              pathname:
                currentPage - 1 === 1
                  ? `${pagePrefix}/`
                  : `${pagePrefix}/page/${currentPage - 1}`,
              query: router.query.s ? { s: router.query.s } : {}
            }}
            className={`${showPrev ? 'text-blue-600 border-b border-blue-400 visible ' : ' invisible bg-gray pointer-events-none '} no-underline pb-1 px-3`}>
            NEWER POSTS <i className='fa-solid fa-arrow-left'></i>
          </SmartLink>
          <SmartLink
            href={{
              pathname: `${pagePrefix}/page/${currentPage + 1}`,
              query: router.query.s ? { s: router.query.s } : {}
            }}
            className={`${showNext ? 'text-blue-600 border-b border-blue-400 visible' : ' invisible bg-gray pointer-events-none '} no-underline pb-1 px-3`}>
            OLDER POSTS <i className='fa-solid fa-arrow-right'></i>
          </SmartLink>
        </div>
      )}
    </div>
  )
}
